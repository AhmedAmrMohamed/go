...
int valid(int a ,int b){return a>=1&&a<=9&&b>=1&&b<=9;}
bool liberity(int a ,int b)
{
	...
	for(int i=0;i<4;i++)
	{
		...
		//cout<<a+dx[i]<<' '<<b+dy[i]<<board[a+dx[i]][b+dy[i]]<<endl;
		 if(board[a+dx[i]][b+dy[i]]=='.')return true;
		// cout<<a<<' '<<b<<endl;
	}
	return false;
}
bool strliberity(int a,int b) //liberity of string that has a,b
{
	char c= board[a][b];
...
void adj(int a,int b,char trplay)//determine which stones belongs to strin that can be captured
{
 ...
 void clcpoints()
 {
	 ...
	 {
		 if(board[i][j]=='X'||board[i][j]=='O'){bfs(i,j,board[i][j]);}
	 }
	 for(int i=1;i<=9;i++)
	 {
		 for(int j=1;j<=9;j++)
...
int main()
{
	...
	string corr;
	int x,spare,y,nx,ny; //true-->x
	bool turn =true,checkinput=true,pass=false,validpass;
	char inp;
	string s="123456789";
	for(int i=0;i<=9;i++){p[0][i+1]=s[i];p[i+1][0]=s[i];}
	...
	while(true)
	{
		...
		// cin>>x;
		// while(x>9||x<1){cout<<"out of the board...\n"; cin >> x;}
